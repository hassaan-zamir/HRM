                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('user.update', $user) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group">
                                    <label class="form-control-label" for="input-permissions">{{ __('User Permissions') }}</label>
                                    @foreach($permissions as $label => $permission)
                                      <br>
                                      <label style="color:#5e72e4;font-size:12px !important;padding-left:20px;" class="form-control-label">{{ $label }}</label>
                                      @foreach($permission as $sub_permissions)
                                        <br>
                                        <input style="margin-left:20px;" type="checkbox" name="granted_permissions[]" class=""  value="{{ $sub_permissions['permission_id'] }}" @if($sub_permissions['checked']) checked @endif>
                                        <label style="font-size:12px !important;padding-left:8px;" class="form-control-label">{{$sub_permissions['key']}}</label>
